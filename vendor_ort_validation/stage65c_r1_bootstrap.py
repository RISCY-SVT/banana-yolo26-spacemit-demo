#!/usr/bin/env python3
"""Shared-draw four-surface COCO bootstrap and A1 provider interaction."""
...
PAIRS = (
    ("A1_EP-vs-B2_EP", "A1_EP", "B2_EP"),
    ("A1_CPU-vs-B2_CPU", "A1_CPU", "B2_CPU"),
    ("A1_EP-vs-A1_CPU", "A1_EP", "A1_CPU"),
...
def main() -> int:
    ...
    if options.replicates < 10000:
        raise ValueError("Stage65C-R1 requires at least 10000 replicates")
    ...
    a1_ep = SURFACES.index("A1_EP")
    a1_cpu = SURFACES.index("A1_CPU")
    b2_ep = SURFACES.index("B2_EP")
    b2_cpu = SURFACES.index("B2_CPU")
    interaction_rows = []
    for metric_index, metric in enumerate(METRICS):
        interaction = (
            values[:, a1_ep, metric_index]
            - values[:, a1_cpu, metric_index]
            - values[:, b2_ep, metric_index]
            + values[:, b2_cpu, metric_index]
        )
        point = (
            _BASES["A1_EP"]["point"][metric]
            - _BASES["A1_CPU"]["point"][metric]
