#!/usr/bin/env python3
"""Normalize Stage65C matched-performance evidence and enforce its gates."""
...
def normalize_abba(performance_root: Path, tracked_root: Path) -> tuple[bool, dict[str, float]]:
    ...
    for samples_file in sorted(performance_root.glob("*/samples.tsv")):
        ...
        match = re.fullmatch(r"(noise|abba)-b([0-9]+)-s([0-9]+)-(A1|B2)", label)
        if not match:
            continue
    ...
    for metric in ("inference", "two_stage"):
        a1 = aggregate[("A1", metric)]
        b2 = aggregate[("B2", metric)]
        a1_stats, b2_stats = stats(a1), stats(b2)
        median_ratio = float(a1_stats["median_us"]) / float(b2_stats["median_us"])
        p95_ratio = float(a1_stats["p95_us"]) / float(b2_stats["p95_us"])
        block_ratios = [
            statistics.median(block_values[(block, "A1", metric)])
        ...
        ratio_rows.append(
            {
                ...
                "a1_samples": len(a1),
                "b2_samples": len(b2),
                "a1_median_us": a1_stats["median_us"],
                "b2_median_us": b2_stats["median_us"],
                "median_ratio": median_ratio,
                "a1_p95_us": a1_stats["p95_us"],
...
def main() -> int:
    ...
    report.write_text(
        "# Stage65C matched performance decision\n\n"
        f"- Decision: `{'pass' if passed else 'fail'}`\n"
        f"- A1/B2 steady-inference median ratio: `{ratios['inference_median_ratio']:.9f}` (gate `<= 1.02`)\n"
        f"- A1/B2 two-stage median ratio: `{ratios['two_stage_median_ratio']:.9f}` (gate `<= 1.02`)\n"
        f"- A1/B2 steady-inference p95 ratio: `{ratios['inference_p95_ratio']:.9f}` (gate `<= 1.05`)\n"
        f"- A1/B2 two-stage p95 ratio: `{ratios['two_stage_p95_ratio']:.9f}` (gate `<= 1.05`)\n"
