#!/usr/bin/env python3
"""Shared-draw paired COCO bootstrap for the DEV-001B candidate matrix."""
...
def main() -> int:
    ...
    parser.add_argument("--seed", type=int, default=65006)
    parser.add_argument("--workers", type=int, default=1)
    parser.add_argument("--output-dir", required=True, type=Path)
    options = parser.parse_args()
    if options.replicates < 10_000:
        raise ValueError("DEV-001B requires at least 10000 shared draws")
    ...
    if len(parsed) != len(parsed_entries) or len(parsed) < 3:
        raise ValueError("surface names must be unique and include controls plus candidates")
    ...
    candidates = [
        surface
        for surface in _SURFACES
        if surface not in {options.baseline, options.pareto_reference}
    ]
    pairs = [
        (f"{candidate}-vs-{reference}", candidate, reference)
        for candidate in candidates
        for reference in (options.baseline, options.pareto_reference)
    ]
