...
STAGE_ID=${STAGE_ID:-BANANA-YOLO26-XSLIM-STAGE65D-C2-FROZEN-K1X-SPACEMIT-EP-BOARD-PASSPORT-FP32-FP16-AND-CUSTOM-ENGINE-APPLICATION-COMPARISON-001}
BOARD=${BOARD:-svt@banana}
BOARD_ROOT="/data/k1x-stage-runs/${STAGE_ID}"
DEV001B_ROOT=/data/k1x-stage-runs/BANANA-YOLO26-XSLIM-DEV-001B-ALL-S8-GENERIC-HARDENING-ADAPTIVE-ROUNDING-BLOCK-RECONSTRUCTION-AND-DETECTOR-PARETO-HOST-GATE-001
...
mkdir -p "$root"/{bin,models,manifests,runtime,opencv,fixtures/tiny,fixtures/fixed,tiny-controls,plugin,profile,h500/images,coco,performance,stability,tmp,cache,state,vendor-control/runners,vendor-control/runtimes/rt206}
...
rsync -a \
  ...
  /data/worktrees/banana-yolo26-xslim211-s8-qdq-validation/vendor_ort_validation/stage65d_run_two_stage_board.sh \
  /data/worktrees/banana-yolo26-xslim211-s8-qdq-validation/vendor_ort_validation/stage65d_run_coco_board.sh \
  "$BOARD:$BOARD_ROOT/bin/"
rsync -a \
  "$DEV001B_ROOT/candidates/run1/C2_T6_RANK_QP/c2_t6_rank_qp.deployable.onnx" \
  "$DEV001B_ROOT/candidates/run1/C2_T6_RANK_QP/c2_t6_rank_qp.inference.onnx" \
  ...
  "$R1_ROOT/postprocess/B2/candidate-gate/B2/models/stage65b_r1_b2.postprocess.onnx" \
  "$S64_ROOT/models/fp32-split/yolo26n_640_e2e_fp32.inference.onnx" \
...
{
  sha256sum "$root/models/c2_t6_rank_qp.deployable.onnx"
  sha256sum "$root/models/c2_t6_rank_qp.inference.onnx"
  sha256sum "$root/models/stage65b_r1_b2_split_s8_qdq.onnx"
  sha256sum "$root/models/stage65b_r1_b2.inference.onnx"
  sha256sum "$root/models/stage65b_r1_b2.postprocess.onnx"
  sha256sum "$root/models/yolo26n_640_e2e_fp32.inference.onnx"
