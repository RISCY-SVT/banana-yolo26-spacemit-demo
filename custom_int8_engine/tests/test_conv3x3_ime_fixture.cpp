...
int run_case(const Y26Conv2DParams& params, std::uint32_t seed) {
    ...
    auto weights =
        y26_make_i8_vector(static_cast<std::size_t>(params.output_c * 3 * 3 * params.input_c), seed + 1U);
    ...
    if (y26_conv3x3_i8s8s32_nhwc_scalar(
    ...
    std::printf("conv3x3_ime shape=%dx%dx%d->%d output=%dx%d status=%d mismatches=%d checksum_scalar=%lld checksum_ime=%lld\n",
                params.input_h,
                params.input_w,
                params.input_c,
                params.output_c,
...
int main() {
    if (!y26_vmadot_4x4x8_ime_available_buildtime()) {
        std::printf("conv3x3_ime skipped: not built with IME\n");
        return 0;
    }
    if (!y26_k1x_ime_hotpath_allowed_on_current_cpu()) {
        std::printf("conv3x3_ime skipped: IME unavailable or not on cluster0\n");
        return 1;
    }

    if (run_case(Y26Conv2DParams{5, 5, 8, 8, 1, 1, 1, 1}, 301U) != 0) {
        return 1;
    }
    if (run_case(Y26Conv2DParams{6, 6, 5, 6, 2, 2, 1, 1}, 302U) != 0) {
        return 1;
    }
    return run_case(Y26Conv2DParams{4, 4, 9, 7, 1, 1, 1, 1}, 303U) == 0 ? 0 : 1;
}
