...
void count_job(void* opaque, int worker, y26::stage49::WorkerScratch&) {
    ...
    const std::uint64_t prior =
        context.observed_epoch[static_cast<std::size_t>(worker)].exchange(
...
int run_lifecycle(int transitions, int constructions) {
    ...
    std::cout << "stage61_worker_lifecycle transitions=" << transitions
              << " constructions=" << constructions
              << " dispatches=" << expected_dispatches
              << " completion_policy=all_workers_complete"
...
int main(int argc, char** argv) {
    ...
    std::thread watchdog([&]() {
        ...
        std::cerr << "stage61 worker lifecycle watchdog timeout\n";
        std::_Exit(124);
    });
