...
int main(int argc, char** argv) {
    ...
    config.node_name = "stage60m/startup-readiness";
    ...
    std::uint64_t worker_readiness_transitions = 0;
    for (int iteration = 0; iteration < iterations; ++iteration) {
        for (int workers = 1; workers <= 4; ++workers) {
            ...
            y26_threaded_conv_destroy(workspace);
            ++workspaces;
            worker_readiness_transitions += static_cast<std::uint64_t>(workers);
        }
    }

    std::cout << "stage60m_threaded_startup iterations=" << iterations
              << " workspaces=" << workspaces
              << " worker_readiness_transitions=" << worker_readiness_transitions
              << " failures=0\n";
    return 0;
}
