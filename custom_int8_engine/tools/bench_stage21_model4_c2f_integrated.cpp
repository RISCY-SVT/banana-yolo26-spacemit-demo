...
long long checksum_i32_stage21(const std::int32_t* values, std::size_t count) {
    long long sum = 0;
...
CandidateSummary run_candidate(const y26_stage16_model4_c2f_fixture::Model4C2fFixture& fixture,
    ...
    Y26Stage16Model4C2fConfig cfg =
        stage16_config_from_fixture(fixture, Y26_ACTIVATION_MODE_STAGE9_RVV_F32_LUT, merge_mode);
    ...
    std::vector<double> repeat_merge;
    for (int repeat = 0; repeat < protocol.repeats; ++repeat) {
        for (int i = 0; i < protocol.warmup; ++i) {
            Y26Stage16TimingUs timing {};
            summary.status = threaded ? y26_stage16_model4_c2f_run_ime_threaded_branch0_cluster0_hotpath(
                                             &cfg, &ws, input, output.data(), 0, &timing)
        ...
        repeat_merge.push_back(repeat_sum.merge_us);
        summary.mismatches += output_mismatches(fixture, ws, output);
        summary.checksum = checksum_i32_stage21(output.data(), output.size());
    }
    ...
    summary.merge = stats_from_values(repeat_merge);
    summary.affinity_ok = !threaded ? 1 : y26_stage16_model4_c2f_threaded_worker_affinity_ok(&ws);
    std::cout << "stage21_result candidate=" << candidate
              ...
              << " mean_merge_us=" << summary.merge.mean
...
int main(int argc, char** argv) {
    ...
    std::cout << "subset=candidate_K_model4_c2f_real_runner_compact_oracle_scope"
              << " note=integration-proof-not-model-fps\n";
    if (!y26_vmadot_4x4x8_ime_available_buildtime()) {
        std::cout << "stage21_result candidate=A0_real_runner_pre_c2_or_reference correctness_status=not_built\n";
        return 0;
    }
    (void)y26_k1x_ime_probe_once();
    const auto& fixture = y26_stage16_model4_c2f_fixture::kSyntheticSeededFixture;
    int failures = 0;
    CandidateSummary a0 = run_candidate(fixture,
                                        protocol,
                                        "A0_real_runner_pre_c2_or_reference",
                                        1,
                                        false,
                                        Y26_STAGE16_MERGE_MODE_A2_FUSED_QDQ_NHWC);
    failures += a0.status == Y26_CONV_STATUS_SUCCESS && a0.mismatches == 0 ? 0 : 1;
    CandidateSummary a4 = run_candidate(fixture,
                                        protocol,
                                        "A4_real_runner_threaded_pre_c2",
                                        4,
                                        true,
                                        Y26_STAGE16_MERGE_MODE_A2_FUSED_QDQ_NHWC);
    failures += a4.status == Y26_CONV_STATUS_SUCCESS && a4.mismatches == 0 && a4.affinity_ok == 1 ? 0 : 1;
    CandidateSummary c2 = run_candidate(fixture,
                                        protocol,
                                        "C2_integrated_model4_c2f",
                                        4,
                                        true,
                                        Y26_STAGE16_MERGE_MODE_C2_SPLIT0_CONCAT_LUT);
    failures += c2.status == Y26_CONV_STATUS_SUCCESS && c2.mismatches == 0 && c2.affinity_ok == 1 ? 0 : 1;
