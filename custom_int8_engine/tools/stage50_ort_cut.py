#!/usr/bin/env python3
"""Build the model4-final-to-model8 B120 performance-diagnostic cut."""
...
OUTPUT_NAME = "/model.8/cv2/act/Mul_output_0_QuantizeLinear_Output"
...
def main() -> int:
    ...
    cut_path = args.out_dir / "model4_preact_to_model8_output_b120_timing.onnx"
    onnx.save(cut, cut_path)

    package_meta = json.loads((args.package / "package.json").read_text(encoding="utf-8"))
    output_id = int(package_meta["model8_output_tensor_id"])
    input_raw = args.package / "oracles" / args.fixture / "tensor_000_nchw_u8.bin"
    output_raw = args.package / "oracles" / args.fixture / f"tensor_{output_id:03d}_nchw_u8.bin"
    input_values = np.fromfile(input_raw, dtype=np.uint8)
    output_values = np.fromfile(output_raw, dtype=np.uint8)
    if input_values.size != 1 * 128 * 80 * 80 or output_values.size != 1 * 256 * 20 * 20:
        raise RuntimeError("Stage50 oracle byte count mismatch")
    input_path = args.out_dir / f"{args.fixture}_model4_preact_nchw_u8.npy"
    integer_output_path = args.out_dir / f"{args.fixture}_model8_integer_contract_nchw_u8.npy"
    np.save(input_path, input_values.reshape(1, 128, 80, 80), allow_pickle=False)
    np.save(integer_output_path, output_values.reshape(1, 256, 20, 20), allow_pickle=False)
