...
ram=/dev/shm/y26-stage61-soak-$$
repeats=${Y26_STAGE61_SOAK_REPEATS:-100}
[[ $repeats =~ ^[1-9][0-9]*$ ]] || {
  echo "Y26_STAGE61_SOAK_REPEATS must be a positive integer" >&2
  exit 2
}
mkdir -p "$output" "$ram"
...
for resolution in "$@"; do
  ...
  set +e
  "$binary" --package "$package" --input "$input" --output "$prefix.raw.tsv" \
  ...
  sha256sum "$binary" "$package/asset_hashes.tsv" "$input" >"$prefix.identities.txt"
  for suffix in raw.tsv summary.txt stderr.txt system.tsv exit-status.txt \
                process-wall.tsv identities.txt; do
    [[ -f $prefix.$suffix ]] && cp "$prefix.$suffix" "$final_prefix.$suffix"
  done
  [[ $status -eq 0 ]] || {
    echo "Stage61 soak failed for R$resolution" >&2
    exit "$status"
  }
  samples=$(awk 'END {print NR - 1}' "$final_prefix.raw.tsv")
  [[ $samples -eq $expected_samples ]] || {
    echo "Stage61 soak row-count mismatch for R$resolution: $samples" >&2
    exit 1
  }
done
