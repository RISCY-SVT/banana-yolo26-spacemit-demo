...
constexpr std::size_t kStage6Alignment = 64;
...
std::size_t expected_weight_count(const Y26Stage6ConvNodeConfig& cfg) {
...
bool conv_config_valid(const Y26Stage6ConvNodeConfig& cfg) {
...
bool config_valid(const Y26Stage6MultiblockConfig* cfg) {
    if (cfg == nullptr || cfg->act0_output_scale <= 0.0f || cfg->act0_output_zero_point_u8 < 0 ||
        cfg->act0_output_zero_point_u8 > 255 || !conv_config_valid(cfg->conv0) ||
        !conv_config_valid(cfg->conv1)) {
        return false;
    }
    const int conv0_oh = output_h_for_kernel(cfg->conv0.params, cfg->conv0.kernel_h);
    const int conv0_ow = output_w_for_kernel(cfg->conv0.params, cfg->conv0.kernel_w);
    return cfg->conv1.params.input_h == conv0_oh && cfg->conv1.params.input_w == conv0_ow &&
           cfg->conv1.params.input_c == cfg->conv0.params.output_c &&
           cfg->conv1.activation_zero_point_u8 == cfg->act0_output_zero_point_u8 &&
           cfg->conv1.input_storage_zero_point_s8 == cfg->act0_output_zero_point_u8 - 128;
}

std::int8_t weight_at(const Y26Stage6ConvNodeConfig& cfg, int oc, int kh, int kw, int ic) {
...
void* allocate_aligned(std::size_t bytes) {
    ...
    return ::operator new(bytes, std::align_val_t(kStage6Alignment));
}

void free_aligned(void* ptr) {
    ::operator delete(ptr, std::align_val_t(kStage6Alignment));
}
...
void timing_reset(Y26Stage6TimingUs* timing) {
    if (timing != nullptr) {
        std::memset(timing, 0, sizeof(*timing));
    }
}

int scalar_raw_dot(const Y26Stage6ConvNodeConfig& cfg,
...
int apply_correction(const Y26Stage6ConvNodeConfig& cfg,
...
int apply_activation_requant(const Y26Stage6MultiblockConfig& cfg, Y26Stage6MultiblockWorkspace& ws) {
    if (ws.conv0_i32 == nullptr || ws.conv1_input_s8 == nullptr) {
        return Y26_CONV_STATUS_INVALID_ARGUMENT;
    }
    const int output_m = output_h_for_kernel(cfg.conv0.params, cfg.conv0.kernel_h) *
                         output_w_for_kernel(cfg.conv0.params, cfg.conv0.kernel_w);
    const int channels = cfg.conv0.params.output_c;
    for (int m = 0; m < output_m; ++m) {
        for (int oc = 0; oc < channels; ++oc) {
            const float acc_scale = cfg.conv0.input_scale * cfg.conv0.weight_scales[oc];
            const float conv_float = static_cast<float>(ws.conv0_i32[m * channels + oc]) * acc_scale;
            const std::uint8_t conv_q =
                quantize_u8_nearest_even(conv_float, cfg.conv0.output_scale, cfg.conv0.output_zero_point_u8);
            const float conv_dq =
                static_cast<float>(static_cast<int>(conv_q) - cfg.conv0.output_zero_point_u8) *
                cfg.conv0.output_scale;
            const float sigmoid = 1.0f / (1.0f + std::exp(-conv_dq));
            const float activated = conv_dq * sigmoid;
            const std::uint8_t act_q =
                quantize_u8_nearest_even(activated, cfg.act0_output_scale, cfg.act0_output_zero_point_u8);
            ws.conv1_input_s8[m * channels + oc] =
                static_cast<std::int8_t>(static_cast<int>(act_q) - 128);
        }
    }
    return Y26_CONV_STATUS_SUCCESS;
}

int validate_run_args(const Y26Stage6MultiblockConfig* cfg,
                      const Y26Stage6MultiblockWorkspace* ws,
                      const std::int8_t* input_nhwc_s8,
...
extern "C" void y26_stage6_multiblock_release(Y26Stage6MultiblockWorkspace* ws) {
    if (ws == nullptr) {
        return;
    }
...
extern "C" int y26_stage6_multiblock_prepare(const Y26Stage6MultiblockConfig* cfg,
    ...
    try {
        ws->conv0_weights = y26_prepacked_conv_weights_create_mmt4d_s8(cfg->conv0.weights_ohwi_s8,
                                                                       &cfg->conv0.params,
                                                                       cfg->conv0.kernel_h,
                                                                       cfg->conv0.kernel_w,
                                                                       cfg->conv0.node_name,
                                                                       cfg->conv0.weight_scales);
        ws->conv1_weights = y26_prepacked_conv_weights_create_mmt4d_s8(cfg->conv1.weights_ohwi_s8,
                                                                       &cfg->conv1.params,
                                                                       cfg->conv1.kernel_h,
                                                                       cfg->conv1.kernel_w,
                                                                       cfg->conv1.node_name,
                                                                       cfg->conv1.weight_scales);
        ws->conv0_workspace = y26_conv_workspace_create(&cfg->conv0.params, cfg->conv0.kernel_h, cfg->conv0.kernel_w);
        ws->conv1_workspace = y26_conv_workspace_create(&cfg->conv1.params, cfg->conv1.kernel_h, cfg->conv1.kernel_w);
        ...
        if (ws->conv0_weights == nullptr || ws->conv1_weights == nullptr || ws->conv0_workspace == nullptr ||
            ws->conv1_workspace == nullptr || ws->conv0_raw_i32 == nullptr || ws->conv0_i32 == nullptr ||
            ws->conv1_input_s8 == nullptr || ws->conv1_raw_i32 == nullptr) {
            y26_stage6_multiblock_release(ws);
            return Y26_CONV_STATUS_INVALID_ARGUMENT;
        }
        ...
        ws->conv0_raw_bytes = conv0_count * sizeof(std::int32_t);
        ws->conv0_i32_bytes = conv0_count * sizeof(std::int32_t);
        ws->conv1_input_bytes = conv0_count;
        ws->conv1_raw_bytes = conv1_count * sizeof(std::int32_t);
        ...
        ws->workspace_bytes = y26_conv_workspace_bytes(ws->conv0_workspace) +
                              y26_conv_workspace_bytes(ws->conv1_workspace) + ws->conv0_raw_bytes +
                              ws->conv0_i32_bytes + ws->conv1_input_bytes + ws->conv1_raw_bytes;
        std::memset(ws->conv0_raw_i32, 0, ws->conv0_raw_bytes);
        std::memset(ws->conv0_i32, 0, ws->conv0_i32_bytes);
        std::memset(ws->conv1_input_s8, 0, ws->conv1_input_bytes);
        std::memset(ws->conv1_raw_i32, 0, ws->conv1_raw_bytes);
        ws->prepared = 1;
        return Y26_CONV_STATUS_SUCCESS;
...
extern "C" int y26_stage6_multiblock_conv0_output_h(const Y26Stage6MultiblockConfig* cfg) {
    return config_valid(cfg) ? output_h_for_kernel(cfg->conv0.params, cfg->conv0.kernel_h) : 0;
}

extern "C" int y26_stage6_multiblock_conv0_output_w(const Y26Stage6MultiblockConfig* cfg) {
    return config_valid(cfg) ? output_w_for_kernel(cfg->conv0.params, cfg->conv0.kernel_w) : 0;
}

extern "C" int y26_stage6_multiblock_conv1_output_h(const Y26Stage6MultiblockConfig* cfg) {
    return config_valid(cfg) ? output_h_for_kernel(cfg->conv1.params, cfg->conv1.kernel_h) : 0;
}

extern "C" int y26_stage6_multiblock_conv1_output_w(const Y26Stage6MultiblockConfig* cfg) {
    return config_valid(cfg) ? output_w_for_kernel(cfg->conv1.params, cfg->conv1.kernel_w) : 0;
}

extern "C" std::size_t y26_stage6_multiblock_conv0_output_count(const Y26Stage6MultiblockConfig* cfg) {
    return config_valid(cfg) ? output_count_for_kernel(cfg->conv0.params, cfg->conv0.kernel_h, cfg->conv0.kernel_w) : 0;
}

extern "C" std::size_t y26_stage6_multiblock_conv1_output_count(const Y26Stage6MultiblockConfig* cfg) {
    return config_valid(cfg) ? output_count_for_kernel(cfg->conv1.params, cfg->conv1.kernel_h, cfg->conv1.kernel_w) : 0;
}

extern "C" int y26_stage6_multiblock_run_scalar(const Y26Stage6MultiblockConfig* cfg,
                                                  Y26Stage6MultiblockWorkspace* ws,
                                                  const std::int8_t* input_nhwc_s8,
    ...
    begin = Clock::now();
    status = apply_activation_requant(*cfg, *ws);
    end = Clock::now();
    if (timing != nullptr) {
        timing->activation_us = elapsed_us(begin, end);
    }
    if (status != Y26_CONV_STATUS_SUCCESS) {
        return status;
    }
    ...
    if (status == Y26_CONV_STATUS_SUCCESS) {
        status = apply_correction(cfg->conv1, ws->conv1_weights, ws->conv1_raw_i32, output_i32_nhwc);
    }
    end = Clock::now();
    if (timing != nullptr) {
...
extern "C" int y26_stage6_multiblock_run_ime_cluster0_hotpath(const Y26Stage6MultiblockConfig* cfg,
                                                               Y26Stage6MultiblockWorkspace* ws,
                                                               const std::int8_t* input_nhwc_s8,
                                                               std::int32_t* output_i32_nhwc,
                                                               Y26Stage6TimingUs* timing) {
    ...
    const auto total_begin = Clock::now();
    int status = validate_run_args(cfg, ws, input_nhwc_s8, output_i32_nhwc);
    if (status != Y26_CONV_STATUS_SUCCESS || ws->conv0_workspace == nullptr || ws->conv1_workspace == nullptr) {
...
extern "C" const std::int32_t* y26_stage6_multiblock_conv0_i32(const Y26Stage6MultiblockWorkspace* ws) {
    return ws != nullptr ? ws->conv0_i32 : nullptr;
}

extern "C" const std::int8_t* y26_stage6_multiblock_conv1_input_s8(const Y26Stage6MultiblockWorkspace* ws) {
