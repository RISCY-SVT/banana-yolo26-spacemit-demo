...
void pack_conv3x3_a_tile(const std::int8_t* input_nhwc,
    ...
    for (int m = 0; m < 4; ++m) {
        ...
        for (int k = 0; k < 8; ++k) {
            const int flat_k = k_offset + k;
            const int kernel_c = params.input_c;
            const int kh = flat_k / (3 * kernel_c);
            const int rem = flat_k % (3 * kernel_c);
            const int kw = rem / kernel_c;
            const int ic = rem % kernel_c;
            const int ih = oh * params.stride_h + kh - params.pad_h;
            const int iw = ow * params.stride_w + kw - params.pad_w;
            dst[m * 8 + k] = (flat_k < 9 * params.input_c)
...
void pack_conv3x3_b_tile(const std::int8_t* weights_oc_kh_kw_ic,
                         const Y26Conv2DParams& params,
                         int n_offset,
                         int k_offset,
                         std::array<std::int8_t, 32>& dst) {
    for (int n = 0; n < 4; ++n) {
        ...
        for (int k = 0; k < 8; ++k) {
            ...
            const int kw = rem / kernel_c;
            const int ic = rem % kernel_c;
            const int weight_index = ((oc * 3 + kh) * 3 + kw) * params.input_c + ic;
            dst[n * 8 + k] = (oc < params.output_c && flat_k < 9 * params.input_c)
                                  ? weights_oc_kh_kw_ic[weight_index]
                                  : 0;
        }
    }
}
...
extern "C" int y26_conv3x3_i8s8s32_nhwc_ime(const std::int8_t* input_nhwc,
    ...
    const int kernel_k = 9 * params->input_c;
    ...
    for (int m0 = 0; m0 < output_m; m0 += 4) {
        for (int n0 = 0; n0 < params->output_c; n0 += 4) {
            ...
            for (int k0 = 0; k0 < kernel_k; k0 += 8) {
                pack_conv3x3_a_tile(input_nhwc, *params, output_w, m0, k0, a_tile);
                pack_conv3x3_b_tile(weights_oc_kh_kw_ic, *params, n0, k0, b_tile);
